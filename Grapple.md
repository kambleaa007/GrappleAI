...
The following implementation uses the standard Python `mcp` SDK (FastMCP) and `anthropic` library, which are the standard for 2026 agentic workflows.
...
Run Server (Testing):
Use the MCP Inspector to verify your tools before connecting the agent:
bash
npx @modelcontextprotocol/inspector python server.py
Use code with caution.

Deploy Agent:
Run the following to start the interaction loop:
bash
python agent.py
Use code with caution.
